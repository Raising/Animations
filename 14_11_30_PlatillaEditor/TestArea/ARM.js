...
ARM.Text.In.turnFromBehind = function(textContainerID,time){
	...
	var totalTime = (animateLineTime+(animateLineprogress*splitText.chars.length));
	var timeCoeficient = timeAux/totalTime;
	return TweenMax.staggerFrom(splitText.chars, animateLineTime*timeCoeficient, { autoAlpha:0,  rotationX:-180,  transformOrigin:"50% 50% -200", ease:Sine.easeOut}, animateLineprogress*timeCoeficient);
}
...
ARM.Container.Mid.levitate = function(containerID,time){
	...
	tbl.fromTo($('#'+containerID),0.7, {x:-10,transformOrigin:"50% 50% 0", ease:Sine.easeInOut}, {x:10,repeat:8,yoyo:true,transformOrigin:"50% 50% 0", ease:Sine.easeInOut})
	tbl.fromTo($('#'+containerID),1.2, {y:-10,transformOrigin:"50% 50% 0", ease:Sine.easeInOut}, {y:10,repeat:5,yoyo:true,transformOrigin:"50% 50% 0", ease:Sine.easeInOut},0.5);
	...
	tbl.to($('#'+containerID),0.2, {scale:1.0,transformOrigin:"50% 50% 0", ease:Sine.easeOut});
	tbl.timeScale(7/time);
	return tbl;
}
