...
EPR.CreatorMenu = function(identificador,interactor,posX,posY){
	...
	this.createNewDiv = function(){
		...
		console.log($(Cmenu.checkInside).prop('checked'));
...
EPR.AnimationSelectorMenu = function(identificador,interactor,posX,posY){
	...
	this.reset = function(){
		$(Amenu.body).empty().append(EPR.GLOBALS.selectedContainer.getAnimationControls());
	}

}

EPR.AnimationTraslation = function(){
	var animation = this;
	this.stats = {};
	...
	this.getHTML = function(){
		TweenMax.to(animation.timeReference,0.5,{width:(animation.stats.duration/EPR.GLOBALS.maxDuration)*220,x:30+(220/EPR.GLOBALS.maxDuration)*animation.stats.startTime});
		return animation.HTMLversion;
	}
}

EPR.AnimationTransformMenu = function(identificador,interactor,posX,posY){
	EPR.Menu.call(this,identificador,interactor,500,400,posX,posY);
	var ATmenu =this;
}
...
EPR.workingDiv = function(name,dimX,dimY){
   ...
   this.getAnimationControls =function(){
	...
	animationAddButton.click(function(){
		...
		var newAnimation = new EPR.AnimationTraslation();
...
EPR.fusionFile = function(insideBody){
